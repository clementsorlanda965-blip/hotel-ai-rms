---
name: impeccable
description: 像素级UI/UX打磨——设计审核、润色、动画、配色、排版、无障碍、性能优化等全方位前端界面提升。覆盖网站、落地页、仪表盘、产品UI、组件、表单、设置页。让平庸设计变出彩，花哨设计变优雅，支持浏览器实时迭代。不适用后端或非UI任务。输入"打磨设计""优化界面""UI审核""设计润色""提升视觉"时触发。
version: 3.1.1
user-invocable: true
...
1. **Summarize**: Present the register identification, sub-command, and action plan to the user
2. **Confirm**: Wait for explicit user approval
3. **Proceed**: Only execute after confirmed

If the user disagrees with the plan, adjust before proceeding.
