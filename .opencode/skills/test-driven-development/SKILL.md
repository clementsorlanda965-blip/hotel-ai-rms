...
description: 测试驱动开发——实现功能或修复bug时必须先写测试再写代码，遵循红-绿-重构循环。输入"TDD""测试驱动""写测试""单元测试"时触发。
---
...
| 位置 | 确认内容 |
|------|---------|
| RED 编写前 | 向用户说明测试意图："我要测试的是：X。确认开始？" |
| RED → GREEN | 展示失败的测试输出，确认用户看到预期失败后开始写代码 |
| GREEN → REFACTOR | 展示全部通过的测试结果，确认进入重构 |
| 重构完成 | 确认重构后测试仍然全绿 |

如果用户拒绝或要求调整预期，按用户要求修改后再继续。
