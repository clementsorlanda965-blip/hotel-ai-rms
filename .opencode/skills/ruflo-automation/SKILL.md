...
## 测试用例
本技能配套测试数据见 `test-prompts.json`，包含 3 个典型场景的 prompt 和预期输出。每次优化后使用该文件验证表现覆盖。测试场景索引：

| 场景 | Prompt | 对应功能 |
|------|--------|---------|
| 1 | 创建酒店价格每日监控 | 二、Worker 派发 |
| 2 | 批量10期大明王朝解说 | 三、Autopilot 无人值守 |
| 3 | 查看上次费用和进度 | 三、Autopilot 进度 + 五、费用查询 |
...
| 你的需求 | 入口 | 跳转 |
|----------|------|------|
| 保存本次产出供下次用 | 一、记忆系统 → 存入记忆 | §1.1 |
| 检索上次做过的内容 | 一、记忆系统 → 检索记忆 | §1.2 |
| 定时爬取数据/监控价格 | 二、后台 Worker → 派发 Worker | §2 |
| 批量 >3 个任务，关电脑离开 | 三、Autopilot → 启用 | §3 |
| 多个 Agent 协作 | 四、蜂巢协同 → 初始化 | §4 |
| 省钱/查看账单 | 五、模型路由 → 路由/费用 | §5 |
| 模拟浏览器操作 | 六、浏览器自动化 → 录制 | §6 |
